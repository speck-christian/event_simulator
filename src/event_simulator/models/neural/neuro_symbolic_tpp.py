...
from ..common.networks import NeuroSymbolicTPPModel
...
class NeuroSymbolicTPPBaseline(LearnedTPPBaseline):
    name = "neuro_symbolic_tpp"
    description = "Rule-guided GRU temporal point process with symbolic condition logic and neural residual correction"
    ...
    def __init__(
        ...
        condition_loss_weight: float = 0.18,
    ) -> None:
                ...
                samples.append(
                    {
                        ...
                        "symbolic_features": torch.tensor(symbolic_condition_feature_vector(state), dtype=torch.float32),
    ...
    def collate(self, batch: list[dict[str, object]]) -> dict[str, torch.Tensor]:
        ...
        collated["symbolic_features"] = torch.stack([item["symbolic_features"] for item in batch])
    ...
    def fit(self, train_runs: list[dict[str, Any]]) -> None:
        ...
        threshold_loader = DataLoader(SequenceDataset(samples), batch_size=self.batch_size, shuffle=False, collate_fn=self.collate)
        state_dim = len(samples[0]["state_features"])
        symbolic_dim = len(samples[0]["symbolic_features"])
        condition_dim = len(self.condition_horizons) * len(CONDITION_NAMES)
        self.model = NeuroSymbolicTPPModel(
    ...
    def predict(self, state: ReplayState, summary: dict[str, Any]) -> tuple[str, float]:
        ...
        with torch.no_grad():
            logits, log_delta, _ = self.model(
                ...
                torch.tensor([symbolic_condition_feature_vector(state)], dtype=torch.float32, device=self.runtime_device),
            )
    ...
    def save_checkpoint(self, path: str | Path) -> None:
        ...
        payload["state_dim"] = int(self.model.state_dim)
        payload["symbolic_dim"] = int(self.model.symbolic_dim)
    ...
    @classmethod
    def load_checkpoint(cls, path: str | Path, device: str = "auto") -> "NeuroSymbolicTPPBaseline":
        ...
        model = cls(
            ...
            condition_loss_weight=float(payload.get("condition_loss_weight") or 0.18),
        )
        model.runtime_device = model.resolve_device()
        model.restore_base_state(payload)
        model.model = NeuroSymbolicTPPModel(
            vocab_size=len(model.label_to_id) + 1,
            state_dim=int(payload["state_dim"]),
            symbolic_dim=int(payload["symbolic_dim"]),
